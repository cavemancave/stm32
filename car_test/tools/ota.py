...
用法（**串口默认 /dev/ttyACM0**，Linux 下 USB-CDC 类设备就是它；CH340/CP210x 一类是
/dev/ttyUSB0，换个口用 `--port /dev/ttyUSB0`，或者设环境变量 `CAR_PORT`。
Windows 上照旧写 `--port COM7`）：
    ...
    python tools/ota.py reboot --boot           # 重启进 Bootloader 恢复台
    python tools/ota.py status                  # 两台电机的里程/位置/故障码
    python tools/ota.py ctrl disable            # 电机失能（默认两台都失能）
    python tools/ota.py ctrl movepos 8191 --motor 2   # 只让 2 号机走 90°
    python tools/ota.py ctrl pwron              # 电机电源上电（PC14 拉高）
    python tools/ota.py ctrl pwcycle            # 断电重启电机（状态卡死时用）
    python tools/ota.py ctrl speedloop --motor 1      # 1 号机切速度环（小车用这个环）
    python tools/ota.py ctrl speed 300 --motor 1      # 1 号机 30.0rpm 正转（0.1rpm 为单位）
    python tools/ota.py ctrl speed 0 --motor 1        # 1 号机停

  两轮差速小车的一键测试框架见 `tools/car.py`（识别左右轮 / 单轮点动 / 差速前进）。
...
# 电脑这边的串口。Linux 下 USB-CDC（ST-Link 的虚拟串口 / 无线模块的 CDC 口）就是
# /dev/ttyACM0，CH340/CP210x 那类一般是 /dev/ttyUSB0。默认给 /dev/ttyACM0，
# 要换就 --port，或者设环境变量 CAR_PORT（Windows 上写 COM7 也照样能用）。
DEFAULT_PORT = os.environ.get("CAR_PORT", "/dev/ttyACM0")

# 电机侧的速度单位：速度环 -3800~3800 对应 -380~380rpm，所以 1 个单位 = 0.1rpm
RPM_TO_RAW = 10


def check_port(port: str) -> None:
    """串口存在性检查：不存在就把系统上能用的口列出来。

    Linux 上「/dev/ttyACM0 不存在」几乎都不是代码问题，而是 ① 设备没插好/认成了
    ttyUSB* ② 当前用户不在 dialout 组里（这时候口存在但打不开，报 PermissionError）。
    """
    ...
    if found:
        # /dev/ttyS0-31 是主板自带的串口（一般不接设备），列表里看着很吵 ——
        # 有 ttyACM/ttyUSB 就只列它们，没有才退回全部（免得什么都没有时看不出来）。
        usb_like = [p for p in found if ("ttyACM" in p.device or "ttyUSB" in p.device)]
        shown = usb_like if usb_like else found[:8]
        lines = "\n".join(f"     {p.device}   {p.description}" for p in shown)
        if not usb_like:
            lines += "\n     （上面都是主板自带的 /dev/ttyS* —— 设备真插好了吗？`dmesg | tail` 看一眼）"
    else:
        lines = "     （一个都没有：USB 线/驱动/模块供电都看一眼）"

    raise OtaError(
        f"串口 {port} 不存在。当前系统上找到的串口：\n{lines}\n"
        f"   提示：USB-CDC 是 /dev/ttyACM*、CH340/CP210x 是 /dev/ttyUSB*；"
        f"用 `--port /dev/ttyUSB0` 指定，或 `ls /dev/tty*` 看一眼。\n"
        f"   能看见口但打不开（Permission denied）：sudo usermod -aG dialout $USER 之后重新登录。")
...
CTRL = {
    ...
    "speed": 0x10,        # 速度环转速给定：单位 0.1rpm、带符号（±3800 ↔ ±380rpm），0 = 停
    "accel": 0x11,        # 速度环加速时间：每 1rpm 多少 ms（0..255，0 按 1 处理）
    "drive": 0x12,        # ★ 两台一起走：arg = (电机2基准<<16)|(电机1基准&0xFFFF)，0.1rpm 带符号；
                          #   第 6 个字节（--motor）当**看门狗**用：单位 50ms，0 = 不启用。
                          #   全 0 = 停车。两条 0x64 是设备侧背靠背发的（起步差 ~10ms）
    "trim": 0x13,         # 里程差闭环（走直线）：arg = 0 关 / 1|2 = 开且这个数就是左轮的电机序号
}
...
def i16(v):
    """把 0..65535 当成有符号 16 位看（转速、trim 这些字段是带符号的）"""
    return v - 65536 if v & 0x8000 else v


def pair_unpack(v):
    """drive 命令的 arg：(低 16 位, 高 16 位) → 两个带符号的 0.1rpm 值"""
    return i16(v & 0xFFFF), i16((v >> 16) & 0xFFFF)


def pair_pack(v1, v2):
    """上面的逆运算：两个带符号的 0.1rpm → drive 命令的 arg"""
...
def cmd_ctrl(dev: Device, args):
    ...
    if cmd == 0x08:                     # 电源命令：data 返回当前状态（0/1）
    ...
    elif cmd == 0x0F:                   # speedloop：data = 复核后的模式值（要 0x02），data2 = 总线 ID
        extra = (f"  模式=0x{data:02X}{'' if data == 0x02 else ' ⚠ 不是 0x02（没切成功）'}"
                 f"  总线 ID={data2}")
    elif cmd == 0x10:                   # speed：data = 生效转速（0.1rpm，带符号），data2 = 总线 ID
        ...
        trim3 = i32(rsp, 9) if len(rsp) > 12 else 0
        if data:
            extra = f"  里程差闭环=开  左轮=电机{data2}  当前trim={trim3 / 10.0:+.1f}rpm"
        else:
            extra = "  里程差闭环=关"
...
def main():
    ...
    ap.add_argument("--port", default=DEFAULT_PORT,
                    help=f"电脑这边的串口，默认 {DEFAULT_PORT}（Linux 的 USB-CDC）；"
                         f"CH340/CP210x 一般是 /dev/ttyUSB0，Windows 是 COM7。"
                         f"也可以设环境变量 CAR_PORT（selftest 不需要这个参数）")
    ...
    p.add_argument("cmd", choices=sorted(CTRL.keys()) + sorted(CTRL_ARG.keys()),
                   help="disable/enable/posloop/movepos/movedeg/speedloop/speed/accel/stop/mute/pause"
                        "/pwr/vmon/cells/follow/spin/stacks/pwron/pwoff/pwcycle/6s/3s/12v/drive/trim")
    p.add_argument("arg", nargs="?", type=lambda s: int(s, 0), default=None,
                   help="参数：movepos=0..32767、movedeg=0..359、mute/pause=0/1、pwr=0/1/2、"
                        ...
                        "spin=转速（0.1°/s，0=停）；speedloop 不用参数；"
                        "speed=转速（0.1rpm，带符号 ±3800，0=停）；accel=每 1rpm 的加速时间 ms；"
                        "drive=pair_pack(电机1, 电机2) 两个 0.1rpm（用 car.py 走这条更省事）；"
                        "trim=0 关 / 1|2 开（这个数就是左轮的电机序号）")
    p.add_argument("--motor", type=int, default=0,
    ...
    try:
        ...
        print("   口存在但打不开 → 多半是权限：sudo usermod -aG dialout $USER 之后重新登录；"
              "或者别的程序（minicom / 上一个脚本）还占着这个口。", file=sys.stderr)
        return 1
    ...
    try:
        ...
        if dev.rx_total == 0:
            print(f"   {args.port} @{args.baud} 一个字节都没收到，优先查：\n"
                  ...
                  f"     2) 串口号对不对（linux: `ls /dev/tty*`、`dmesg | tail`），"
                  f"模块有没有配对/上电",
                  file=sys.stderr)
        return 1
    except KeyboardInterrupt:
