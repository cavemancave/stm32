...
> 这一段是**台架标定/位置环**那套（按键触发）。要做**两轮差速小车**（速度环、按命令
> 单独控制两台电机、识别左右轮）看后面的
> [「两轮差速小车（速度环）」](#两轮差速小车速度环) 一节 + `tools/car.py`。

这一版先不跑 IMU（同一条 USART10 单总线上挂 2 台电机：ID1 / ID2），业务已经全部搬进
FreeRTOS 任务：
...
| 任务 / 对象 | 优先级 | 说明 |
| --- | --- | --- |
| `defaultTask` → `MotorCtrl_Task` | Normal | 主控制流程（按键那套台架标定） |
...
| `motorFollow` | Normal | 位置跟随（同样懒创建，见「位置跟随」一节） |
...
| 电源电压监测（PC4/ADC1_INP4） | Boot 打两行（英文）：`[power] VCC_IN = 23.87V, cell 3.97V (6S), ~77%` + `[power]   ADC1_INP4/PC4 raw=43092, divider 11:1, VREF=3300 mV, full scale 36.30V`；`ctrl vmon` → 23870 mV、`status` 末尾带电压/单片/剩余百分比（**电机没答上也打印**）；`ctrl 12v` → 过压告警、`ctrl 6s` → 恢复正常；`ctrl cells 12` 被夹到 8 |
| 两台电机各走一圈（每步 90°） | 先 `ctrl pwron` + 两台 `ctrl enable` + `ctrl posloop`（都复核 `mode=0x03`），再 `movedeg 90/180/270/0` 各 4 步：8 条全部 `reply=OK`，结束时两台**里程都是 1 圈**、回到起始角度（348.2° / 346.0°），fault=0x00 |
...
**串口默认 `/dev/ttyACM0`**（Linux 上 USB-CDC 类设备就是它；CH340/CP210x 那类一般是
`/dev/ttyUSB0`）—— 下面省掉了 `--port`，要换口就 `--port /dev/ttyUSB0`，
或者设环境变量 `CAR_PORT`。（Windows 上照旧 `--port COM7`。口不存在时脚本会把系统上
能用的串口列出来；能看见口但打不开是权限问题：`sudo usermod -aG dialout $USER` 后重新登录。）
...
python tools/ota.py status                  # 电机里程/位置/故障码 + 电源状态 + 电压
python tools/ota.py ctrl disable            # 电机失能（enable/posloop/movepos/... 同理）
python tools/ota.py ctrl pwron              # 电机电源上电（PC14 拉高）
python tools/ota.py ctrl pwoff              # 电机电源断电
python tools/ota.py ctrl pwcycle            # 断电重启电机（状态卡死时用）
python tools/ota.py ctrl vmon               # 读电源电压（总压 + 单片）
python tools/ota.py ctrl 6s                 # 按 6S 算单片（现场那套电池）
python tools/ota.py ctrl 12v                # 按 3S 算单片（12V 开关电源那套）
```
...
⚠ **从 Windows 换到 Linux 之后，`build/Debug` 里是 Windows 的路径**（`CMakeCache.txt`、
`build.ninja` 里写着 `C:\Users\...\cmake.exe`），直接 `ninja` 会报
`/bin/sh: 1: C:Userstaishan...cmake.exe: not found`。清一次再重新 configure 就行
（`build/` 全是生成物，删掉不心疼）：

```bash
rm -rf build/Debug && cube-cmake --preset Debug && ninja -C build/Debug
```
...
2. 自己新增的驱动**不要**直接写进生成文件，统一走「独立 cmake 片段」模式：
   - 配置放在 `cmake/bmi088.cmake`、`cmake/ws2812.cmake`、`cmake/motor.cmake`、
     ...
     `cmake/power_mon.cmake`、`cmake/ota.cmake`
...
* 无线读取：`ctrl vmon`（总压 mV + 电池串数 + **剩余百分比**）；`status` 回帧末尾也会带上
  电压 / 单片 / 告警标志 / 剩余百分比。
...
- 无线命令用 `--motor` 指定哪一台。**不填**时按安全默认：失能/急停作用于**全部**、
  ...
  ```bash
  python tools/ota.py status                          # 两台都问
  python tools/ota.py ctrl enable  --motor 2          # 只使能 2 号机
  python tools/ota.py ctrl posloop --motor 2          # 2 号机切位置环
  python tools/ota.py ctrl movepos 8191 --motor 2     # 2 号机走 90°
  python tools/ota.py ctrl disable --motor 2          # 只失能 2 号机（不切电源）
  python tools/ota.py ctrl disable                    # 两台都失能 + 切电源
  ```
...
```bash
python tools/ota.py ctrl follow        # ① 开始跟随（默认 20 ms 周期；会先上电+切位置环）
python tools/ota.py ctrl spin 900      # ② 让 2 号机以 90.0°/s 自动转（0.1°/s 为单位，0=停）
python tools/ota.py ctrl follow 0      # ③ 停止（回帧里带实测周期和平均跟随误差）
```
...
小车底盘就是"两台电机 + 一条总线"，控制目标是**让轮子以给定转速一直转**。这一节是
2026-09-22 为小车加的那套东西：怎么让单台电机按转速跑、怎么先搞清楚哪台是左轮。

### 电机以哪个环运行？—— 轮子用**速度环（0x02）**

| 环 | 0x64 给定值的含义 | 对"轮子"合适吗 |
| --- | --- | --- |
| 开环 `0x00` | 电压 | ❌ 完全没闭环，负载一变转速就飞 |
| 电流环 `0x01`（使能后的默认） | 力矩（±32767 ↔ ±4A） | ❌ 给的是力气不是转速：左右轮负载不一样就跑不直，上坡/压地毯会掉速，要定速得自己再做一套速度 PID |
| **速度环 `0x02`** | **转速（±3800 ↔ ±380rpm，1 单位 = 0.1rpm）** | ✅ **就是它**：电机内部自己闭环，给多少 rpm 就转多少 rpm，左右轮给一样的值就能走直；给 0 就是停 |
| 位置环 `0x03` | 目标角度（0~32767 ↔ 0~360°） | ❌ 给定值是"某一圈里的一个角度"，而且**按最短路径**走、接近目标时慢慢爬（实测 90° 要 1.7~2s 才停稳）。只适合"转到位就停"的关节（比如机械臂/云台），轮子会走一小段就停 |

所以：**小车用速度环，位置环留给机械臂那种"转到位"的场合**。（位置环那套代码、
按键往返测试都还在，别删 —— 台架标定、验证零点还用得上。）

差速底盘的两个自由度，用速度环表达起来非常直接：
...
（注意：位置环做不到"一直转"这件事 —— 位置值一圈是 0~32767 且 `32767 ≡ 0`，
发个大数值它只会按最短路径走到那个角度就停。）

### 用命令控制（`tools/car.py`）

`tools/car.py` 是给小车搭的测试框架，跑在 `ota.py` 的协议层上面。串口默认
`/dev/ttyACM0`（`--port` 或 `CAR_PORT` 可换）：

```bash
python tools/car.py setup                                  # ① 上电 + 两台使能 + 切速度环
python tools/car.py status                                 #    两台在哪台/什么模式/位置/里程/故障码
python tools/car.py spin --motor 1 --rpm 20 --seconds 2     # ② 只让 1 号机 20rpm 转 2 秒
python tools/car.py spin --motor 2 --rpm -20 --seconds 2    #    只让 2 号机反着转
python tools/car.py watch --seconds 5                       #    看两台实际转速（编码器差分算的）
python tools/car.py drive --left 20 --right 20 --seconds 3  # ③ 两轮一起（按映射）
python tools/car.py stop                                    # ④ 停车
python tools/car.py stop --disable                          #    收工：停车 + 失能 + 断电机电源
```

- **`setup` / `spin` / `drive` 都必须在同一个进程里自己收尾**：速度环下给了转速电机就会
  一直转，所以每个动作都是"给转速 → 等 → 一定给 0 停下"，Ctrl+C 也会在 `finally` 里停。
  但**第一次测试请把轮子架空、手别离开电源开关**。
- `setup` 还会做两件“小车专用”的事：把**左轮是几台**告诉设备（里程差闭环要用，见下一节），
  并把 200 ms 状态轮询停掉（闭环自己会读位置，两边抢总线只会都变慢）。
- `drive`（两台一起）走的是**一条** `ctrl drive` 命令 —— 设备侧背靠背发两条 `0x64`，
  两台起步差从 30~80 ms 降到 ~10 ms；`spin`（只动一台）仍然用 `ctrl speed`。
- `spin` 结束会打里程/位置的变化 —— **正 rpm 时位置和里程该往正方向走**，反了说明这个
  电机的相序/映射要反（记到映射里就行）。
- `watch` 的转速是拿 `(里程 x 32768 + 位置)` 的差分算的（差分按最短路径折一下，处理
  位置翻圈）。给 20rpm、`watch` 里读到 ±20 上下，说明速度环在工作、编码器刻度也对。
- ⚠ **跑小车测试的时候别按板上的 USER_KEY**（PA15）：按键那套会把**两台**电机使能并切到
  **位置环**去走来走去的台架测试，和"用命令开小车"是两条路（撞上之后 `ctrl speed` 会发现
  模式不对、自己把速度环切回来，但中间那几秒很乱）。按键流程是台架标定用的，台架上保留。

### 先搞清楚哪台电机是左轮（`car.py id`）

**"左" = 从车尾看着车头方向时的左手边**（驾驶员左手边）。台架上还没装车、或者不确定
接线顺序时：

```bash
python tools/car.py id              # 依次点动两台，然后问你是哪边轮子动了
```

它会：上电 + 两台切速度环 → **电机1 正转 1.5 s（停一下）→ 电机2 正转 1.5 s** → 停下来
**逐台**问两个问题（"电机N 转的时候是哪一侧的轮子动了" + "那侧轮子是往前还是往后"），
然后把结果写进 `tools/car_map.json`：

```json
{ "left": 2, "right": 1, "invert_left": false, "invert_right": true }
```

`invert_xxx = true` 的含义是「那台电机**给正 rpm 时是把车往后推**的」，`drive` 会自动取反 ——
**所以两台电机是镜像装的（一个正转往前、一个正转往后）不用改接线，改这个文件就行。**
不想跑交互提问就手写，还带 `--no-invert-xxx` 可以取消反向：

```bash
python tools/car.py map --left 2 --right 1 --invert-right     # 左轮=电机2、右轮=电机1（要反向）
python tools/car.py map --left 1 --right 2 --no-invert-left   # 改回来
```

⚠ 左右是以**站在车尾、朝车头看**为准的。定完映射先验两条（车架空或留出两米）：

```bash
python tools/car.py drive --left 20 --right 20 --seconds 1.5   # 该往车头方向直走
python tools/car.py drive --left 20 --right -20 --seconds 1.5  # 该原地左转（车头转向左边）
```

- 直行**往后走** → 两侧的 `invert` 都要反过来（`map --invert-left --invert-right`，
  或者从"两边都反"改成"两边都不反"用 `--no-invert-left --no-invert-right`）；
- 直行对、但原地转**转反了** → 说明左右认反了，把 `--left/--right` 对调、两个 `invert` 也跟着对调
  （例如 `map --left 1 --right 2 --invert-left --no-invert-right`）。

2026-09-22 那台车实测：**电机1 → 右轮，正 rpm 往后；电机2 → 左轮，正 rpm 往前**，
所以映射是 `{"left": 2, "right": 1, "invert_left": false, "invert_right": true}`（两台电机是
镜像装的，所以有一侧必须反向 —— 这是正常的，不用改接线）。

映射定了之后，`drive --left 20 --right 20` 应该**直着走**、`drive --left 20 --right -20`
应该**原地转**。这两条对不上就说明左/右或者方向记错了，回 `id` 重来。

### 键盘遥控（`tools/car_teleop.py`）—— 按住就走，松手就停

```bash
python tools/car_teleop.py --speed 30        # w/a/s/d 前后左右，空格急停，q 退出
```
...
| `w` / `s` | 前进 / 后退（两轮同速） |
| `a` / `d` | 原地左转 / 原地右转（一正一负） |
| `w`+`a`、`w`+`d` | 一边前进一边拐（按键可组合，按住不放就一直是那个动作） |
| `空格` | 急停（按住期间一直给 0） |
| `q` | 退出（退出前会停车） |

**"松手就停"在终端里没有天然的事件**：终端只在**按下**（和自动重复）时发字符，**松开什么都不发**。
所以脚本两条路都备着，跑起来时会打印用的是哪一种：

| 模式 | 怎么工作 | 用在哪些终端 |
| --- | --- | --- |
| **kitty 键盘协议**（`CSI > 10 u`） | 按下/重复/抬起三个事件都带上类型，抬起**立刻**停 | VS Code 终端、kitty、wezterm 等 |
| **自动重复超时推断** | 一直按着的话终端会一直重复发那个字母，超过 `--release-ms`（默认 250ms）没再收到就算松手 | 不支持上面协议的终端 |

另外还有**看门狗**兜底：每条 DRIVE 命令自带"超过 `--watchdog-ms`（默认 400 ms）没收到新命令
就把两台停掉"。所以松手、无线断、模块掉电、笔记本睡死，车都会自己停，而不是指望最后那条
stop 命令发得出去。（这也**顺便把"电机断联保护"做掉了**，而且比开 `0xA0/0x10` 那套更好使：
那是电机自己的 3 秒计时，我们这里 400 ms 就停。）

- 发送频率默认 **20 Hz**（`--rate`，即每 50 ms 一条命令）；方向反了回 `car.py id` 重认映射。
- 车停着的时候脚本**不再刷命令**（目标已经是 0，不需要心跳），按第一个键时会立刻发。
- ⚠ 第一次先架空 / 用 `--speed 15` 试。

### 走直线：里程差闭环（在设备侧跑，25 ms 一拍）

两台电机**各自**的速度环给同样的给定值，转速并不会真的相等（环路静差、轮径、摩擦、地面都不一样），
所以开环差速"跑两米还行、跑十米一定歪"。这一版把"走直线"做成**车级闭环**，放在设备里：

```
每 25 ms：读两台 0x74 的位置 → 增量之差 (Δs_左 − Δs_右) → 积分（带泄漏）→ trim
          v_左 = 基准_左 − trim ,  v_右 = 基准_右 + trim     （只在变化 ≥1 个单位时才发 0x64）
```

- **为什么闭环放设备侧**：上位机每拍都要过一趟无线（10~30 ms 抖动），闭环会被链路抖死；
  设备侧本来就在总线旁边。一拍 = 2×`0x74` + 2×`0x64` ≈ 24 ms（10 字节帧 @38400 单程 2.6 ms），
  占空比 ~50%，和位置跟随那套同量级（它能跑到 40~50 Hz），放得下。
- 全是**整数定点**（不用浮点）：`trim = e/55`（546 计数 ≈ 10 秒的 0.1rpm 差 → 补 10 个 0.1rpm），
  积分泄漏 `e -= e/64`（时间常数 ≈1.6 s），输出限幅 ±120（±12 rpm）。
- ⚠ 位置**只用"最短路径增量"累加**，不用 `0x74` 的里程字段 —— 里程在位置过零点时会掉一整圈
  （位置跟随那边踩过，见 `motor_follow.c` 里 `MotorFollow_ShortestDiff` 的注释）。
- **只在"直行类"动作下积分**（两侧同号，且速度差 ≤ 快的那侧的 1/4）：转圈/明显走弧时航向本来就
  在变，拿里程差去修会跟驾驶指令打架，所以那两种情况下自动停积分并把 trim 归零。
- 顺带把**起步差**也补掉了：以前"先动的那侧"多走的那点，会被积分项在随后 1~2 秒里还给另一侧。
- 看设备日志的 `[drive]` 行（1 Hz）：`rate` = 实测拍频、`e` = 里程差积分、`trim` = 当前修正量、
  `sent` = 真正发出去的（基准 ± trim）。`car.py drive` 收尾还会打两台各走了多少计数、以及
  **左−右** 差多少 —— 那就是这一段歪了多少。
...
| 命令 | 码 | 干什么 |
| --- | --- | --- |
| `ctrl speedloop [--motor N]` | `0x0F` | 上电 → 使能 → `0xA0/0x02` 切速度环 → **用 `0x75/0x76` 复核模式值真是 0x02**（和切位置环同一个坑：不认这个模式的固件也会回一个合法的 `0xA1`，光看"有没有回帧"会误判）。回帧 `data` = 复核到的模式值 |
| `ctrl speed <±3800> [--motor N]` | `0x10` | 速度环给转速，**单位 0.1rpm**（`300` = 30.0rpm，负数反着转，`0` = 停）。回帧 `data` = 生效转速 |
| `ctrl accel <0..255> [--motor N]` | `0x11` | 速度环加速时间：每 1rpm 多少 ms（`0` 按 1 处理）。值越大起步越柔：`0→30rpm` 用 `accel x 30ms`。上电默认 **10**（0.3 s 起） |
| `ctrl drive <pair> --motor <wd>` | `0x12` | **两台一起走**：`arg` = `pair_pack(电机1, 电机2)`（各 0.1rpm 带符号，全 0 = 停车），第 6 个字节 = **看门狗超时**（单位 50 ms，0 = 不启用）。设备侧**背靠背**发两条 `0x64`；回帧 `data` = 原样回两个基准、`data2` = 看门狗 ms |
| `ctrl trim <0｜1｜2>` | `0x13` | 里程差闭环：`0` 关，`1`/`2` 开并且这个数就是"左轮"的电机序号。回帧 `data` = 开没开、`data2` = 左轮、`data3` = 当前 trim（0.1rpm） |

代码：`motor_ctrl.c` 里 `MotorCtrl_PrepareSpeedLoop / SetSpeed / SetSpeeds / GetAccel /
SetAccel`；**新模块** `Device/Motor/motor_drive.c`（闭环 + 看门狗任务，新 cmake 片段
`cmake/motor_drive.cmake`）；命令码在 `ota_layout.h` 往后接（`0x0F`~`0x13`，回帧结构没动，
老上位机不受影响）。

三个特意改的地方（都是实测逼出来的）：

1. **不再每条命令都查一次模式**（`ctrl speed` 原来先发一次 `0x75`）：改成内存里一个
   `s_in_speed_loop[]` 缓存，确认过一次就只发 `0x64`。⚠ 只有**会让电机离开速度环**的动作
   才清它：失能、断电/断电重启、进 OTA、切进位置环；另外 `0x64` 收不到合法 `0x65` 也会清
   （兜底：下次自动重新确认）。省掉的是一趟总线往返（~5 ms + HAL/队列开销），
   **而且这是"两台一起起步"的前提** —— 要是 `0x75` 夹在两条 `0x64` 中间，起步差就从 ~10 ms
   放大成"一次查询那么长"。
2. **`drive` 一条命令管两台，两条 `0x64` 背靠背**：实测两台最多差 30~80 ms（= 两次无线往返 +
   每台一次模式查询 + 上位机循环），低速时就是"先动的那侧把车拽歪一下"（20 rpm、60 mm 轮、
   轴距 130 mm 时 50 ms 差 ≈ 车头歪 1.4°）。合成一条之后只剩总线串行的 ~10 ms。
3. **`MotorPwr_OnAndSettle()` 不再每次都白等 500 ms**：它原来无条件延时，遥控 20 Hz 下每条
   命令都要等半秒（命令全堆在 otaSvc 任务里，实测会从 20 Hz 掉到 2 Hz）。现在**本来就是开着的
   就直接返回**，只有真的刚上电那一次才等。

⚠ **还没做的**：`trim` 只管"两台走一样多"，管不了**打滑**和**绝对航向**（轮子空转时里程差
是假的）。要更稳就得上板上的 BMI088（现在 `#if 0` 关着）：陀螺 Z 积分出 yaw 做航向保持，
输出还是这个差分项。另外速度环下电机"给了就一直转"，所以**别让车跑出急停开关范围**。
...
- 日志现在从 **USART1（PA9/PA10，接无线模块）** 出去，同时镜像到 **UART7（PE7/PE8）**；
  **USART1 = 921600**、**UART7 = 115200**，都是 8N1（UART7 内核时钟 = D2PCLK1 = 120 MHz →
  ...
  ⚠ Bootloader 与 App 共用 `OTA_PORT_BAUD`，但它**不参与 OTA** ⇒ 改了波特率要用 ST-Link 重烧一次
    `motor_boot.hex`（2026-09-19 已烧）。现在两者都是 921600，升级时 BL 那几行 `bootloader: …`
    也是直接可读的；只有还在跑**旧 BL** 的板子才会看到乱码，那时进恢复台加 `--baud 115200`。
  ⚠ **接回无线模块之前**：两侧模块的“串口波特率”都要设成 921600（用模块的配置工具），
    否则 App 会听不见。⚠ 2026-09-19 调波特率时台架上接的是**有线**串口（链路上没有模块），
    所以“模块自己就跟上了”这个结论**不成立**，接无线前必须把模块那一侧也确认/改掉。
- **日志风格约定（2026-09-19 起）**：
