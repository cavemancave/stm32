...
**实测吞吐与耗时分解**（同一块板子，镜像 ~113 KB）：

| 配置 | 实测吞吐 | 整次升级 |
| --- | --- | --- |
| 115200 + 1024 字节/块（初版） | 4.2 KB/s | 28.2 s |
| 115200 + 4096 字节/块 | 6.4 KB/s | 18.8 s |
| 921600 + 4096 字节/块 | 12.7 KB/s | 10.3 s |
| 再修掉上位机读串口的白等 | 18.0 KB/s | 7.3 s |
| **再只擦镜像占用的扇区**（当前） | **26.6 KB/s** | **5.3 s** |

**各项实测（DWT 周期计数 + 分段计时 —— 这些数不要凭感觉猜）**：

| 项 | 实测 | 说明 |
| --- | --- | --- |
| 扇区擦除 | **0.97 s / 128 KB 扇区** | 384 KB（3 扇区）实测 2.9 s；而一个 113 KB 的镜像只占 **1** 个扇区 ⇒ 以前白白多等 ~2 s |
| 写 flash | **98 µs / flash word**（32 B） | 整个镜像 3541 个字 = **0.35 s**，只占 5%（`blank` 检查另 3.2 µs/字） |
| 串口传输 | 113 KB @921600 ≈ **1.3 s** | 每包 4096 B 的帧 ≈ 45 ms |
| 上位机读串口 | 每包曾白搭 **~110 ms** | `read(256)` 会等满或超时；帧已解出还把 `pump(0.1)` 剩下时间走完 |

> **结论（按实际收益排序）**：
> ① **上位机读串口的方式**（~3 s，最大也最容易被忽略）：`pyserial` 的 `read(n)` 会等到读满 n 字节
> **或者超时**才返回 ⇒ 一条 19 字节的回复每次都要把剩下的超时（50 ms）耗光；`wait_frame()` 又用
> `pump(0.1)`，帧已经解出来了还要把剩下 100 ms 走完。改成"有多少读多少（`in_waiting`）+ 收到帧立刻返回"。
> ② **只擦必要的扇区**（~2 s）：`OtaFlash_EraseSlotForSize()` 按 `ceil(size/128 KB)` 擦 1~3 个扇区
> （安全性论证见该函数注释）。
> ③ 加大块（1024 → 4096，`OTA_CHUNK_*`）：往返次数除以 4 ⇒ ~1.5×。
> ④ 提高波特率（115200 → 921600，`OTA_PORT_BAUD`）：每块串口时间 94 → 45 ms ⇒ ~1.4×。
>
> ⚠ **写 flash 从来不是瓶颈**：98 µs/字（≈3 µs/字节）比"擦"（7.4 µs/字节）还快。
> 早期版本按"每包固定开销"反推、把它当成 5.9 s 的大头 —— 上机用 DWT 一量就被推翻了，
> 所以这条留在这儿当反面教材：**先量再优化**。
>
> 还能再快的地方：协议从"停等"改成"窗口 4~8 块 + 累积确认"（`next_off` 天然支持累积确认，
