...
enum
{
    ...
    OTA_CTRL_FOLLOW       = 0x0BU,   /* 位置跟随：2 号机当遥控端、1 号机跟着转（详见 motor_follow.h）。
                                        arg = 0 停止 / 其它 = 跟随周期 ms（< 5 按默认 20 算）。
                                        回复 data = 1 在跑、data2 = 实测周期 µs、
                                        data3 = 最近一秒的平均跟随误差（计数，带符号） */
    OTA_CTRL_SPIN         = 0x0CU,   /* 让 leader（2 号机）自动匀速转：arg = 转速（0.1°/s），
                                        0 = 停（目标冻结、它就地在位置环上停住）。
                                        只能在跟随运行中发（驱动器是跟随循环里那个斜坡）。
    ...
    OTA_CTRL_LEAD         = 0x0EU,   /* 速度前馈提前量（ms）：arg = 0..2000，0 = 纯跟随。
                                        位置环在匀速下有个正比于速度的稳态误差
                                        （实测 42°/s ⇒ 22°，折合 ~0.5 s），提前量取到
                                        那个等效延迟就能把它压掉。可在跟随运行中直接调。
                                        回复 data = 生效值（ms） */
