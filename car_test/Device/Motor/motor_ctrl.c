/**
  ...
  *          外加状态轮询
  *
  * ⚠ 这里其实有**两条路**，用的是两个不同的环，别混：
  *   - 按键那套（MotorCtrl_Task）= **位置环（0x03）** 的台架标定测试："转到位就停"；
  *   - 无线命令 `ctrl speedloop/speed/accel` = **速度环（0x02）**，给两轮差速小车用：
                            ...
                            uint32_t *out, uint32_t *out2, uint32_t *out3)
{
    ...
    /* 跟随在跑的时候：作用于 **follower（1 号机）或“全部”** 的手动命令要先停跟随，
       ...
       （DRIVE 是两台一起、后面跟着的看门狗字节会让序号对不上，所以单独列出来） */
    if ((MotorFollow_IsRunning() != 0U) &&
        (cmd != OTA_CTRL_FOLLOW) && (cmd != OTA_CTRL_SPIN) && (cmd != OTA_CTRL_LEAD) &&
        ((cmd == OTA_CTRL_DRIVE) || (motor_index == 0U) ||
         (motor_index == MOTOR_FOLLOW_FOLLOWER_INDEX)))
    {
