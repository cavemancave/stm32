...
/* 挂在同一条 USART10 上的电机台数（= motor_ctrl.c 里 motor_ids[] 的元素个数）。
   ⚠ 达妙这颗的 ID 是**上电时锁存**的（ID 脚 低 = ID1 / 高 = ID2），所以一条总线最多两台。
   无线命令里的"电机序号"是 1..MOTOR_COUNT（= motor_ids[] 下标 +1），不是总线 ID。
   放公共头是因为别的模块（motor_drive.c）要按它开数组 —— 改这里的同时改 motor_ids[]。 */
#define MOTOR_COUNT               2U

/* 速度环给定值的量纲：-3800 ~ 3800 ↔ -380 ~ 380rpm，也就是 **1 单位 = 0.1rpm**。
   无线命令 `ctrl speed <v>` 的 v 就是这个单位（带符号，0 = 停）。
   ⚠ 别和位置环的 0~32767（0~360°）混了：同一个 0x64 数值在不同环里含义完全不同。 */
#define MOTOR_SPEED_MAX_RAW       3800
...
/* ---- 速度环（两轮差速小车用这个环） ----
 *
 * 为什么轮子用速度环：位置环的给定值是"某一圈里的一个角度"（按最短路径走、接近目标
 * 时慢慢爬，实测 90° 要 1.7~2s 才停稳），只适合"转到位就停"的关节；轮子要的是
 * "一直以某个转速转下去"。电流环给的是力矩，转速随负载漂、左右轮不一致就跑不直。
 * 速度环给定多少 rpm 就转多少 rpm（电机内部闭环），左右轮给一样的值就能走直。
 */

/* 使能 + 切速度环（0xA0/0x02）+ **复核模式值**。
   返回复核到的模式值：MOTOR_MODE_SPEED(0x02) = 成功；其它值 / MOTOR_MODE_UNKNOWN = 失败。
   ⚠ 不看 0xA1 回帧里的值：那是"切换之后的实际模式"，不认 0x02 的固件也会回一个合法 0xA1。 */
uint8_t MotorCtrl_PrepareSpeedLoop(uint8_t motor_id);

/* 速度环给转速：0x64 给定值 = speed_0p1rpm（单位 0.1rpm，带符号，0 = 停）。
   模式检查是**缓存**的（第一次/失能后/断电后/切过位置环才查一次 0x75），
   所以高频调用（遥控 20 Hz）也不用每次多花一趟总线往返。详见 motor_ctrl.c 里
   s_in_speed_loop 的注释。
   返回 1 = 收到合法 0x65 反馈；0 = 失败（原因在日志里） */
uint8_t MotorCtrl_SetSpeed(uint8_t motor_id, int16_t speed_0p1rpm);

/* 多台**一起**给转速：speeds[i] 对应 motor_ids[i]（= 命令里的 1 / 2 号机，不是总线 ID）。
   两条 0x64 是**背靠背**发出去的（中间不查模式、不打长日志），所以两台起步差只有
   总线串行的 ~10ms —— 小车"一起起步"就用这个，别连发两次 MotorCtrl_SetSpeed。
   返回 1 = 全部收到合法 0x65 */
uint8_t MotorCtrl_SetSpeeds(const int16_t *speeds, uint8_t count);

/* 把"已在速度环"的缓存全部作废（断电/重启/OTA 之后调；失能已经自己清了） */
void    MotorCtrl_ForgetSpeedLoopAll(void);

/* 速度环的加速时间（0x64 的 DATA[6]，每 1rpm 多少 ms；0 按 1 处理）。
   值越大起步越柔和：0→30rpm 用 accel x 30ms。上电默认 10（= 0.3s 起）。 */
