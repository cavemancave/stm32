...
| **新的无线口** | **USART1：PA9 = TX、PA10 = RX**（模块侧 UART0），115200 8N1 | 全双工，日志 + 控制帧 + OTA 三合一 |
...
| 项目 | 值 |
| --- | --- |
| 口 | USART1：PA9=TX、PA10=RX，AF7，115200 8N1（`OTA_PORT_BAUD` 可改，两边要一致） |
...
| 0x10 | `CTRL_CMD` | H→D | `cmd(1) arg(4)` | `data(4)`（见 §5.4） |
| 0x11 | `CTRL_STATUS` | H→D | — | `mileage(4) position(2) fault(1) mode(1)` |
...
`CTRL_STATUS` 是只读快照（里程/位置/故障码/模式），比 `0x74` 那套「一问一答 + 文本日志」更适合
上位机做曲线。
...
**吞吐估算**（115200 = 11.5 KB/s 理论上限）：

| 链路 | 单块往返 | 实测吞吐 | 67 KB 固件 | 256 KB 固件 |
| --- | --- | --- | --- | --- |
| USB-TTL 直连 | ~0 ms | ~10 KB/s | ~7 s | ~26 s |
| 蓝牙透传（连接间隔 15~30 ms） | ~40 ms | ~7 KB/s | ~10 s | ~37 s |
| WiFi 透传（TCP） | ~5 ms | ~9.5 KB/s | ~8 s | ~28 s |

> 想更快就两条路：① 提高波特率（模块支持 460800/921600 的话，改 `OTA_PORT_BAUD` 就行）；
> ② 把停等改成「窗口 4~8 块 + 累积确认」——协议里 `next_off` 字段天然支持累积确认，
