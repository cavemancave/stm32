...
| 配置 | 单块往返 | 实测吞吐 | 整次升级 |
| --- | --- | --- | --- |
| 115200 + 1024 字节/块（初版） | ~140 ms | 4.2 KB/s | 28.2 s |
| 115200 + 4096 字节/块 | ~140 ms | 6.4 KB/s | 18.8 s |
| **921600 + 4096 字节/块**（当前） | ~140 ms | **12.7 KB/s** | **10.3 s** |

> **关键结论：每块的往返延迟（~140 ms）几乎和块大小无关**，所以提速要两条一起走：
> ① 加大块（1024 → 4096，`OTA_CHUNK_*`）把往返次数除以 4 ⇒ 1.5×；
> ② 提高波特率（115200 → 921600，`OTA_PORT_BAUD`）把每块的串口时间从 94 ms 压到 12 ms ⇒ 再 2×。
> **只调波特率大约只有 1.5×**（前期 115200 的传输时间只占总时间的一部分），
> 两者叠加才把 28.2 s 干到 10.3 s（2.7×）。
> 想再快就只能改协议了：把停等改成「窗口 4~8 块 + 累积确认」——`next_off` 字段天然支持累积确认，
> 设备侧只要允许多收几块（乱序写 Flash 是安全的，每 32 字节独立），属于后续优化项。
