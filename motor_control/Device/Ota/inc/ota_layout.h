...
/* ---- 无线口 ----
   **921600 = 115200 × 8**（2026-09-19 改）。H7 这边没精度问题：USART1 走 D2PCLK2 = 120 MHz，
   ...
   ⚠ 这个值必须和**无线模块的串口波特率**对得上，否则就是“设备听不见”：
     1) [已完成] 本宏 = 921600、已重新构建并烧进 App（实测：手里这对模块**不用改**，自己就跟上了；
        换到不会自动识别的模块，才需要把两侧模块的串口波特率都改成 921600）；
     2) 升级固件时，如果设备跑的还是旧固件（旧波特率），那一次要 `--baud 115200`。
   ⚠ Bootloader **不参与 OTA**：没重烧 `motor_boot` 之前它一直是旧宏的波特率，
     所以“升级时 BL 那几行 banner 是乱码”是正常的（App 的日志不受影响）；
   ...
   ⚠ 实测：瓶颈是“每包的往返延迟 ~140 ms”而不是串口带宽，所以不要指望 ×8 能快 8 倍
     —— 真正的大头是把块大小从 1024 加到 4096（见上面 OTA_CHUNK_*）。 */
