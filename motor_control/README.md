...
| 升级速度实测 | 112988 字节镜像：1024 块/115200 **4.2 KB/s、28.2 s** → 4096 块/115200 **6.4 KB/s、18.8 s** → 4096 块/921600 **12.7 KB/s、10.3 s** → 再修掉上位机读串口的白等后 **18.0 KB/s、7.3 s**（累计 3.9×） |
...
- 日志现在从 **USART1（PA9/PA10，接无线模块）** 出去，同时镜像到 **UART7（PE7/PE8）**；
  **USART1 = 921600**、**UART7 = 115200**，都是 8N1（UART7 内核时钟 = D2PCLK1 = 120 MHz →
  ...
  ⚠ 升级时 **BL 那几行 banner 是乱码**是正常的 —— Bootloader 是不参与 OTA 的独立镜像，
    它那份还是按 115200 编的（App 的日志不受影响）；要用 BL 恢复台就加 `--baud 115200`，
    或者拿 ST-Link 把 `motor_boot` 也重烧一遍。
  ⚠ 无线模块得能跟上 921600：实测手里这对**不用改**（自己就跟上了）；
    换到不会自动识别的模块，两侧模块的“串口波特率”都要改成 921600，否则 App 会听不见。
- **日志风格约定（2026-09-19 起）**：
